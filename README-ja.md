...
**これは** AI CLI（Claude Code / Codex / Pi / Gemini CLI / opencode）向けの tmux セッションマネージャ。やることは三つ：
...
- Claude Code / Codex / Pi / Gemini CLI / opencode のうち少なくとも一つ（atm はそれらが `~/.claude/projects/`、`~/.codex/sessions/`、`~/.pi/agent/sessions/`、`~/.gemini/tmp/*/chats/`、`~/.local/share/opencode/opencode.db` に書くデータを読むだけ）
