...
10. **一个配置项不能依赖「可能永远不会被写」的块。** `restore.on-boot` 原来把 resurrect 的钩子写在
    atm 的持久化块里。而那个块在用户自己管 tpm 时**整块不写**（atm 不动用户写的东西）—— 于是那些机器上
    这个设置停在 `true` 却什么都不做，而工具打出的提示（「跑一次 `atm install`」）保证没用。
    把死状态报出来只是一半;块必须独立出来。能独立是因为 resurrect 读这个选项的时机是**恢复发生时**
