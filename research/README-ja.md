...
9. **`MemoryHigh` はスロットル弁で、kill のしきい値ではない。slice 名の `-` はパス階層。** 2026-09-10 に二つ同時に踏んだ。どちらも `dispatch.py` に記録済み：
   - セッション単位の `MemoryHigh` 既定 2G の根拠は「この上限で何割が **kill** されるか」の分布だった ——
     しかし `MemoryHigh` はプロセスを kill しない。**メモリ確保のたびにカーネルが同期回収する**だけ。
     実測のセッションピークが 4.7GB あるのにソフト上限を 2G にすると、正常に働く長時間セッションが永久に
     スロットリングされる：生きている、エラーも出ない、`oom_kill` は 0、なのに固まったように遅い。
     小メモリ機での実測：`high=2G` に対し `current=2633M`、`high` イベント **227 万回**。
     セッション単位の上限は「誰を犠牲にするか」を決めるだけで、**合計は slice が守る**。
     その分担に合わせて値を決める（現在は既定 `auto`）。
     **しかも両方の層を読み戻すこと**：小メモリ機で先に上限に当たるのは**合計**のほう —— 5 つの scope が
     それぞれ自分のソフト上限以下でも、合計 4725M が slice の 4096M を超え、全部が回収で遅くなる。
     scope だけ見ると「ソフト上限に達したセッションはありません」と出てしまう。
   - **systemd は slice 名の `-` をパス階層として扱う**：`atm-ai.slice` は
     `user@UID.service/atm.slice/atm-ai.slice` にあり、`user@UID.service/atm-ai.slice` ではない。
     フラットなパスを見ると、3 つの scope が動いていても 1 つも見つからない。さらに `MemoryHigh=infinity` の後は
     `memory.high` が数値ではなく文字列 `max` を返す —— int として解析すると落ちるが、これは**診断**コードで、
     最も落ちてはいけないものだ。

10. **設定項目が「書かれないかもしれないブロック」に依存してはならない。** `restore.on-boot` は
    resurrect のフックを atm の永続化ブロック内に書いていた。そのブロックは tpm をユーザー自身が
    管理している場合まるごとスキップされる（atm はユーザーが書いたものに触らない）—— 結果、その種の
    マシンでは設定が `true` のまま何もせず、ツールが出す案内（「`atm install` を実行」）も効かなかった。
    死んだ状態を報告するのは半分の対処でしかなく、ブロックを独立させる必要があった。独立できる根拠は、
    resurrect がこのオプションを読むのは**復元が起きる時**（`helpers.sh:execute_hook` の中で
...
- 2026-09-18 **ペインの健康状態**（`atm health`、サイドバーの `⚠` 表示、詰まりログ `~/.local/state/atm/health.jsonl`）：
  ...
  理由は落とし穴 11。
- 2026-09-18 **詰まり監視プロセス**（0.11.1）：0.11.0 の通知はサイドバーの中でしか動かず、ユーザーはサイドバーを
