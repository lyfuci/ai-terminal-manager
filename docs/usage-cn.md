...
| `prefix + m` | **格子状态栏**：每格顶边右侧显示它卡没卡（`✓` / `⚠回收 high 464/s` …），再按一次收起。顶掉的是 tmux 自带的 `m`（标记格子），`M` 不动。换键：`atm config keys.health` |
...
只查同一个 CLI、同一个目录（和 `claude -r` 自己的查找范围一样），而且会话名得是命令行上的最后一个词 ——
后面还跟着别的词时（存档里引号已经丢了，分不清名字到哪为止）会标成「认不准」跳过。
...
跑某些命令时一个格子会像卡死一样——进程活着、不出字、Ctrl-C 也没反应。现在 atm 会告诉你是哪格、卡在什么上，
用的是内核本来就按格子记着的数（每个 tmux 格子是一个独立的 systemd scope）：

- **侧栏**：卡住的格子前面标红色 `⚠`——`⚠回收`（反复撞内存软上限，CPU 全耗在回收上）、`⚠卡D`
  （进程连续两次采样都在不可中断睡眠）、`⚠超限`（超过软上限）、`⚠内存` / `⚠IO` / `⚠CPU`（PSI 等待时间）。
  选中它，底栏给出原因。
- **不开侧栏也有提醒**：`atm install` 在它的 tmux 块里加一行 `run-shell -b '… atm health --watch'`，
  tmux server 一起来就在后台起一个盯梢进程（对正在跑的 server 也立刻起）。某格**刚开始**卡时，每个连着的
  客户端状态栏都提醒一次。它自己保证只留一份，server 没了几秒内自己退，`atm update` 之后自己换成新版本。
  从 0.11.0 升上来要跑一次 `atm install -y`——没跑的话 `atm update` 和 `atm doctor` 会提醒。
- **统计**：每一段卡顿（开始、结束、时长、原因）记进 `~/.local/state/atm/health.jsonl`。盯梢进程和侧栏
  共用一把「记录员」锁，同一次卡顿只记一次、只提醒一次。

```bash
atm health            # 此刻卡着的格子 + 最近 7 天各格合计
atm health --all      # 正常的格子也列出读数
atm health --days 1   # 换统计窗口；--json 给脚本用
```

`atm doctor` 里也有同样一段。为什么要看回收速率而不只看 PSI：实测一个被 `MemoryHigh` 压着的进程慢了约
640 倍，memory PSI 却只有 1–2%——它在忙着回收，不是在等。细节见 [reference.md](reference.md#格子健康哪格在卡2026-09-18)。
