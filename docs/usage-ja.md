...
| `prefix + m` | **ペインのステータス表示**：各 pane の上枠の右に詰まり状態（`✓` / `⚠回収 high 464/s` …）、もう一度押すと消える。tmux 標準の `m`（pane のマーク）を置き換える；`M` はそのまま。キー変更：`atm config keys.health` |
...
- **サイドバー**：詰まったペインに赤い `⚠` を付ける——`⚠回収`（メモリのソフト上限に当たり続け、CPU が回収に消える）、
  ...
  （PSI の待ち時間）。選ぶとフッターに理由が出る。
- **サイドバーなしでも通知**：`atm install` が自分の tmux ブロックに `run-shell -b '… atm health --watch'` を 1 行加え、
  tmux サーバーの起動と同時に小さな監視プロセスが立ち上がる（動いているサーバーでもすぐ起動）。詰まり**始めた**ときに
  接続中の全クライアントのステータス行で一度だけ知らせる。常に 1 つだけ動き、サーバーが消えると数秒で終了し、
  `atm update` の後は自分で新しい版に入れ替わる。0.11.0 から上げた場合は `atm install -y` を一度実行——
  していなければ `atm update` と `atm doctor` が知らせる。
- **統計**：詰まりごと（開始・終了・時間・原因）を `~/.local/state/atm/health.jsonl` に記録する。監視プロセスとサイドバーは
  「記録係」ロックを共有するので、同じ詰まりは一度だけ記録・通知される。
