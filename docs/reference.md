...
- **会话名**：参数后面不一定是 id，常常是 `/rename` 或 `claude -n` 起的名字。规划时先按 id 查索引（来源必须一致），
  ...
  **只有参数后面就这一个词**时才拿它当名字，而且必须和索引里**原样**的名字（`raw_name`，不经清洗截断）完全相等。
  ...
  索引只读会话文件的头尾，中间的改名看不见，`raw_name` 可能过期：按名字认出的候选投递前再整份重扫一遍，
  确认那仍是**最后一次**改名（`restore.latest_raw_name`）。过期的剔除 —— 结果最多是「没找到」，不会认错。
