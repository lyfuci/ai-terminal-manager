...
- **会话名**：参数后面不一定是 id，常常是 `/rename` 或 `claude -n` 起的名字。规划时先按 id 查索引（来源必须一致），
  ...
  名字取参数后面到第一个选项（`-x` / `--`）之前的**整段**，必须和索引里的名字**完全相等** ——
  存档里引号已经丢了，`claude -r my project` 里的 `project` 可能是名字的一半，也可能是 prompt，分不清就不配；
  这段字经过 `clean_title(limit=40)` 会变（超宽截断、合并空白）也不配，因为索引里的名字是截断过的。
